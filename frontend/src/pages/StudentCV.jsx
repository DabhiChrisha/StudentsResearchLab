...
export default function StudentCV() {
  ...
  const hasMemberStatus =
    isStudentMember !== undefined &&
    isStudentMember !== null &&
    isStudentMember !== "-" &&
    isStudentMember !== "";
  const hasBranch = hasValue(branchVal);

  // Check if ALL basic details are missing
  const allBasicMissing =
    !hasLinkedin && !hasEmail && !hasMemberStatus && !hasBranch;

  // Format student member status
  const memberStatusDisplay = () => {
    if (!hasMemberStatus) return null;
    if (typeof isStudentMember === "boolean")
      return isStudentMember ? "Yes" : "No";
  ...
  if (notFound || !cvData) {
    return (
      <div className="min-h-screen bg-[#0b0f0e] flex flex-col items-center justify-center font-sans text-white">
        <h1 className="text-xl font-bold mb-4 uppercase tracking-widest">
          Profile Not Found
        </h1>
        <p className="text-slate-400 text-sm mb-6">
          No CV data is available for this member in the database.
        </p>
        <Link
          to="/researchers"
          className="text-secondary hover:text-white flex items-center gap-2 border-b border-secondary/20 pb-1 font-bold transition-all"
        >
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#e3eef0]/80 backdrop-blur-md border-b border-black/5">
        <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/"
              className="text-xs font-bold text-slate-900 hover:text-secondary transition-colors uppercase tracking-widest"
            >
              Home
            </Link>
          </div>
          ...
          <section className="mb-20">
            ...
            <div className="flex flex-wrap gap-3">
              {researchAreas.map((area, i) => (
                <span
                  key={i}
                  className="px-4 py-2 rounded-full bg-white/60 border border-secondary/10 text-slate-700 text-sm font-semibold"
                >
                  {area}
                </span>
              ))}
          ...
          <section className="mb-20">
            ...
            <div className="space-y-6 pl-2">
              {patents.map((patent, i) => {
                const isValidPatent =
                  typeof patent === "object"
                    ? patent.title && patent.title.trim() !== ""
                    : patent && patent.trim() !== "";
                ...
                return (
                  <div key={i} className="group relative">
                    <div className="absolute -left-4 top-2 bottom-2 w-[2px] bg-secondary/0 group-hover:bg-secondary/50 transition-all rounded-full" />
                    <h3 className="text-base font-black text-slate-900 tracking-tight mb-1 group-hover:translate-x-1 transition-transform">
                      {typeof patent === "object" ? patent.title : patent}
                    </h3>
                    {typeof patent === "object" &&
                      (patent.status || patent.patent_number) && (
                        <p className="text-sm font-medium text-slate-500 leading-relaxed max-w-prose">
                          {[patent.status, patent.patent_number, patent.year]
                            .filter(Boolean)
                            .join(" • ")}
                        </p>
                      )}
                  ...
                  <div key={i} className="group relative">
                    ...
                    <h3 className="text-base font-black text-slate-900 tracking-tight mb-1 group-hover:translate-x-1 transition-transform">
                      ...
                      {typeof project === "object" && project.url && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-blue-500 ml-2 text-sm underline font-normal"
                        >
                          [Link]
                        </a>
                      )}
                    ...
                    {typeof project === "object" &&
                      (project.description || project.tech_stack) && (
                        <>
                          {project.tech_stack && (
                            <p className="text-xs font-semibold text-secondary/70 mb-1">
                              {project.tech_stack}
                            </p>
                          )}
                          <p className="text-sm font-medium text-slate-500 leading-relaxed max-w-prose">
          ...
          <section className="mb-20">
            ...
            <div className="flex flex-wrap gap-3">
              {hackathons.map((hack, i) => {
                const isValidHack =
                  typeof hack === "object"
                    ? hack.name && hack.name.trim() !== ""
                    : hack && hack.trim() !== "";
                if (!isValidHack) return null;
                return (
                  <span
                    key={i}
                    className="px-4 py-2 rounded-full bg-white/60 border border-secondary/10 text-slate-700 text-sm font-semibold"
                  >
                    {typeof hack === "object" ? (
                      <>
                        {hack.name} {hack.year && `(${hack.year})`}{" "}
                        {hack.result && `- ${hack.result}`}
                      </>
                    ) : (
                      hack
                    )}
          ...
          <section className="mb-20">
            ...
            <div className="space-y-4">
              {extraFields.map(([key, value]) => (
                <div
                  key={key}
                  className="p-6 rounded-2xl bg-white/40 border border-black/5"
                >
